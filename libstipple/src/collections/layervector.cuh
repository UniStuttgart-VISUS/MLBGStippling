...
public:
    ...
    inline LayerVector& operator=(LayerVector<U, J, O> const& other) {
        ...
        m_size = other.m_size;
        m_capacity = other.m_capacity;
        m_layerIndices = other.m_layerIndices;
        m_data = other.m_data;
        return *this;
    }
};
...
#pragma nv_diag_suppress = restrict_qualifier_dropped
        cuda_unwrap(cudaMalloc(&m_layerIndices, m_capacity * sizeof(I)));
        cuda_unwrap(cudaMalloc(&m_data, m_capacity * sizeof(T)));
#pragma warning(suppress : 4068)
#pragma nv_diag_default = restrict_qualifier_dropped
    }

    __host__ inline ~LayerVector() {
        cuda_unwrap(cudaFree(m_layerIndices));
    ...
    __host__ inline void reserve(int capacity) {
        ...
        cuda_unwrap(cudaMemcpy(newLayerIndices, m_layerIndices, m_size * sizeof(I), cudaMemcpyDeviceToDevice));
        cuda_unwrap(cudaMemcpy(newData, m_data, m_size * sizeof(T), cudaMemcpyDeviceToDevice));
        cuda_unwrap(cudaFree(m_layerIndices));
        cuda_unwrap(cudaFree(m_data));
        m_capacity = capacity;
        m_layerIndices = newLayerIndices;
        m_data = newData;
    }
    ...
    __host__ inline void copyToDevice(InputIt begin, InputIt end, UnaryOperation unaryOp) {
        int index = 0;
        std::vector<I> hostLayerIndices(m_size);
        ...
        cuda_unwrap(cudaMemcpy(m_layerIndices, hostLayerIndices.data(),
            m_size * sizeof(I), cudaMemcpyHostToDevice));
        cuda_unwrap(cudaMemcpy(m_data, hostData.data(),
    ...
    __host__ inline void copyToHost(BinaryOp binaryOp) const {
        std::vector<I> hostLayerIndices(m_size);
        std::vector<T> hostData(m_size);
        cuda_unwrap(cudaMemcpy(hostLayerIndices.data(), m_layerIndices, m_size * sizeof(I), cudaMemcpyDeviceToHost));
        cuda_unwrap(cudaMemcpy(hostData.data(), m_data, m_size * sizeof(T), cudaMemcpyDeviceToHost));
